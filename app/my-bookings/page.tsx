...
export default function MyBookingsPage() {
  ...
  // Requirements 22, 28, 77, 78: Real-time user-specific Firestore subscription
  useEffect(() => {
    setIsMounted(true);
  ...
  // Tab Filtering (Requirements 19, 20, 21)
  const filteredBookings = bookings.filter((b) => {
    ...
    const showDate = new Date(`${b.date} ${b.time}`);
    const isPast = showDate.getTime() < Date.now();
  ...
  const handleDownload = async (booking: Booking) => {
    ...
    if (ok) {
      setToastMessage('Ticket JPG downloaded!');
  ...
  const handleShare = async (booking: Booking) => {
    ...
    if (res.success && res.method === 'clipboard') {
      ...
      setToastMessage('Downloaded Ticket JPG!');
      setTimeout(() => setToastMessage(null), 3000);
    }
  };

  // Requirements 20, 21: Archive / Unarchive Action
  const handleToggleArchive = async (booking: Booking) => {
    ...
    setToastMessage(newStatus ? 'Booking moved to Archived' : 'Booking unarchived');
    setTimeout(() => setToastMessage(null), 3000);
  };
  ...
  return (
    <div className="min-h-screen pt-24 pb-20 bg-background">
      ...
      <div className="container mx-auto px-4 md:px-6 max-w-5xl">
        ...
        {/* 3 TABS: UPCOMING, PAST, ARCHIVED (Requirement 19) */}
        <div className="flex bg-secondary p-1 rounded-xl w-full max-w-md mb-10 border border-white/10 font-bold text-xs">
          <button
        ...
        {filteredBookings.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-20 text-center bg-secondary/30 rounded-2xl border border-white/5">
            ...
            <h2 className="text-2xl font-bold font-heading mb-2">No {activeTab} bookings found.</h2>
            <p className="text-muted-foreground mb-8">Your movie passes and event tickets will appear here.</p>
            ...
            {filteredBookings.map((booking, i) => {
              ...
              return (
                ...
                >
                  ...
                  {/* ACTION BAR: VIEW PASS, DOWNLOAD JPG, SHARE TICKET, ARCHIVE/UNARCHIVE (Requirements 14, 17, 19, 20, 21) */}
                  <div className="flex flex-wrap items-center gap-2 pt-4 border-t border-white/10 justify-between">
                    ...
                    <div className="flex items-center gap-1">
                      <button
                        ...
                        className="text-xs text-muted-foreground hover:text-white p-1.5 rounded-full hover:bg-white/10 transition-colors flex items-center gap-1 font-semibold"
